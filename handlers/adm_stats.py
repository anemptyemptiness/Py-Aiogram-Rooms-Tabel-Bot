...
async def report_money_in_range(date_from: str, date_to: str, msg: Message):
    ...
    places = {
        ...
        "Город Рязанка Жетоны": sum([row[0].count("Город Рязанка Жетоны") for row in rows]),
        "Косино Парк Жетоны": sum([row[0].count("Косино Парк Жетоны") for row in rows]),
        "Мякинино": sum([row[0].count("Мякинино") for row in rows]),
    }
